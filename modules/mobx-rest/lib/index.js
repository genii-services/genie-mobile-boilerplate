...
var Model = /** @class */ (function (_super) {
    ...
    Model.prototype.fetch = function (_a) {
        ...
        var _b = apiClient().get(this.url('get', data), data, otherOptions), abort = _b.abort, promise = _b.promise;	//*.method에 따라 다른 url 호출을 위한 확장
        promise
            .then(function (data) {
            if (!data)
                return;
    ...
    Model.prototype.save = function (attributes, _a) {
		...
		var method = this.isNew ? "post" : patch ? "patch" : "put" //*.최적화
        ...
        var _e = apiClient()[method](this.url(method, data), data, __assign({ onProgress: onProgress }, otherOptions)), promise = _e.promise, abort = _e.abort;
        promise
            .then(function (data) {
            if (!data)
                return;
    ...
    Model.prototype.destroy = function (_a) {
        ...
        var _c = apiClient().del(this.url('del'), data, otherOptions), promise = _c.promise, abort = _c.abort;
        if (optimistic && collection) {
            collection.remove(this);
        }
        promise
